...
var packCss = function ($, projectRoot, pageName) {
    ...
    var cssMd5 = md5.update(minCssCode)
        .digest('hex')
        .substring(0,6);
    fs.writeFileSync(path.resolve(cwd, './__publish__/' + projectRoot + '/styles/' + pageName + '-min_' + cssMd5 + '.css'), minCssCode);
    return cssMd5;
};
...
var publishOnePage = function (pageUrl, config, allPackName) {
    ...
    var jsMd5 = packJsCode(config, projectRoot, pageName, scriptmain);
...
var createHtmlPage = function ($, projectRoot, pageName, scriptMain, cssMd5, jsMd5) {
    ...
    $('head').append('<link rel="stylesheet" href="./styles/' + pageName + '-min_' + cssMd5 + '.css"/>');
    $('body').append('<script type="text/javascript" data-main="' + scriptMain.replace('.', '') + '" src="scripts/' + pageName + '-min_' + jsMd5 + '.js" async></script>');
...
var packJsCode = function (config, projectRoot, pageName, scriptmain) {
    ...
    fs.writeFileSync(path.resolve(cwd, './__publish__/' + projectRoot + '/scripts/' + pageName + '-min_' + jsMd5 + '.js'), jscode);
    return jsMd5;
};
