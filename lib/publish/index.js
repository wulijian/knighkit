...
var createHtmlPage = function ($, projectRoot, pageName, scriptMain, cssMd5, jsMd5) {
    ...
    var charset = $('script[role=debug]').attr('charset') || '';
    $('script[role=debug]').remove();
    $('head').append('<link rel="stylesheet" href="' + concatServerBaseUrl + '/styles/' + pageName + '_' + cssMd5 + '.css"/>');
    $('body').append('<script type="text/javascript" charset="' + charset + '" data-main="' + scriptMain.replace('.', '') + '" src="' + concatServerBaseUrl + '/scripts/' + pageName + '_' + jsMd5 + '.js" async></script>');
...
var packJsCode = function (projectRoot, pageName, scriptmain) {
    ...
    if (concatServerBaseUrl === config.serverUrl) { //发布到服务器阶段，删除未压缩的文件
