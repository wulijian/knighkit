...
exports.buildTemplate = function (filedir, value) {
    ...
    if (fs.existsSync(path.join(templateDir, 'index.js'))) { //如果含有index.js 优先使用index方式解析模块
        ...
        if (/___template___\((.*)\)/.test(tmpIndex)) {
            ...
            var templateCode = 'var _data = ' + paramOut + ';' + render.print_to_string()
                    .replace(/^\s*function\s*anonymous\(_data\)\s*\{/, '')
                    .replace(/\s*\}\s*$/, '');
            //模块模版的语法树
            var mtAST = uglify.parse(tmpIndex.replace(/return\s*___template___\((.*)\)/, templateCode));
            compCode = mtAST.print_to_string(generateOptions);
        }
        return compCode;
    } else {
        ...
        var JsCode = tp.getTp('html').directCompile(require('./alterCode').moduleCode(properties))({name: value});
