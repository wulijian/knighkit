...
/**
 * debug阶段转换合成以后的js代码中图片的路径，将相对于js文件的路径转换为相对于主页的路径
 ...
 */
var alterImgPath = function (tempName, code) {
    code = code.replace(/<(img)\s+[\s\S]*?["'\s\w\/]>\s*/ig, function (m) {
        var getSrc = m.match(/(?:\ssrc\s*=\s*)(['"]?)([^'"\s]*)\1/);
        var imgSrc = getSrc[2];
        if (imgSrc.indexOf('http') === 0 ||//网上的资源，直接返回，不处理
            imgSrc === '') {//空的图片地址不处理
            return m;
        }
        var imgRealPath = path.join(config.configs.template, tempName, imgSrc);
        return m.replace(imgSrc, imgRealPath.replace(/\\/g, '\\\\'));
    });
    return code;
};
...
exports.buildTemplate = function (filedir, value) {
    ...
    return alterImgPath(value, tp.getTp('html').directCompile(alterCode.moduleCode(properties))({name: value})); //将模块名称传到模块的模板中
};
