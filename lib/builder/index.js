...
var extendLayout = function (realPath) {
    var extendedCode;
    tp.getSupportedSuffix().forEach(function (suffix) {
        if (nfs.existsSync(realPath + '/index.' + suffix)) {
            var extendCode = nfs.readFileSync(realPath + '/index.' + suffix, 'utf-8').toString();
            if (extendCode.trim().indexOf('amod.extend') === 0) {
                var firstline = extendCode.split('\r\n')[0];
                var layout = firstline.replace('amod.extend', '')
                    .replace(/[\s]*/g, '');
                layoutMod = {
                    layout: layout,
                    suffix: suffix
                ...
                var layoutfrom = path.resolve(realPath, layout) + '/index.' + suffix;
...
var build = function (realPath) {
    ...
    return fs.read(realPath + '/index.js')
        .then(function (code) {
            var render;
            var extendedCode = extendLayout(realPath);
            if (extendedCode !== undefined) {
                var tempIndexMod = realPath + '/__index.' + layoutMod.suffix;
                nfs.writeFileSync(
                    tempIndexMod,
                    extendedCode
                );
                render = tp.compileAdaptive(realPath, '__index');
                nfs.unlink(tempIndexMod);
            } else {
                render = tp.compileAdaptive(realPath, 'index');
            }
            code = code.replace(
                /___template___/,
