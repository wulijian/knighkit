 ...
 * @param next connect的下一步
 */
var responsePuzzledPage = function (rootpath, req, res, next) {
    if (/text\/html/.test(req.headers.accept)) { //url请求的是 text/html
        var relPath = path.join(rootpath, req.url);
        if (!/\.html|\.htm/.test(req.url)) { //默认读取index.html 或者 index.htm
            relPath = path.join(rootpath, req.url, 'index.html');
            if (!fs.existsSync(relPath)) { //检查是否存在 index.html 不存在，取值 index.htm
                relPath = path.join(rootpath, req.url, 'index.htm');
            }
        }
...
exports.startHttpServer = function (rootpath, httpPort, templateRoot) {
    var con = connect()
        .use(function (req, res, next) {
            ...
            return responsePuzzledPage(rootpath, req, res, next);
