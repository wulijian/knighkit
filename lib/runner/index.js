...
var responsePageMapping = function (rootpath, req, res, next) {
    if (/text\/html/.test(req.headers.accept)) { //url请求的是 text/html
        var routers = config.configs.routers;
        var targetUrl = req.url;
        routers.forEach(function (router) {
            if (router.url instanceof RegExp) {
                var matchedUrl = req.url.match(router.url);
                if (matchedUrl !== null) {
                    targetUrl = router.target.replace(/\{\$(\d)\}/, function ($0, $1) {
                        return matchedUrl[$1];
                    });
                }
            } else {
                if (router.url === req.url) { //链接相同
                    targetUrl = router.target;
                }
            }
        });
