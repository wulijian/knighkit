...
define(function (require, exports, module) {
    ...
    var responseBuildEditedModule = function (rootpath, req, res, next) {
        ...
        if (req.url.indexOf('__projects__') === 1 && /index\.js(\?t=\d*)?$/.test(req.url)) {
            var modsrc = req.url.replace(/\/__projects__/g, '');
            var modsource = (rootpath + '/' + modsrc).replace(/index\.js(\?t=\d*)?$/, '');
    ...
    var watchFileUpdateAndEmitToBrowser = function (rootpath, socket) {
        ...
        require('watch').createMonitor(rootpath, {interval: 150}, function (monitor) {
            ...
            monitor.on("changed", function (file, curr, prev) {
                var type = '';
                var fileName = path.relative(rootpath, file).replace(/\\/g, '/');
