...
var template = path.resolve(__dirname, require('./package.json').template);
...
var addDataToMjson = function (realPath, metaName, data) {
    ...
    info.logt('Append [' + metaName + '.json/' + path.basename(realPath) + '] data into [' + realPath + '.json] success.');
};
