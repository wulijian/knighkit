...
define(function (require, exports, module) {
    ...
    if (typeof window !== 'undefined') {
        path = (typeof window !== 'undefined') ? require('../path') : require('path');
        $ = (typeof window !== 'undefined') ? require('../../jquery') : require('jquery');
        require('../q');
        q = Q;
    } else {
        q = require('q');
    ...
    var parseSync = function (htmlCode, subModules, defer, curModule) {
        ...
        syncSubs.forEach(function (sub) {
            ...
            syncRender.push(
                require(path.resolve(path.dirname(curModule), sub.module) + MAIN)//todo:path 需要实现简单的浏览器版本,另外，如果不是相对路径，这里会出错
                    .render(data)
