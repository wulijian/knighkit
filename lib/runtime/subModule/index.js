...
//todo：seajs只有1.3版本可用，循环依赖直接把requirejs搞挂

define(function (require, exports, module) {
    var q , path, $, targetRoot, amods;
    if (typeof window !== 'undefined') {
        path = require('../path');
        $ = require('../../jquery');
        require('../q');
        targetRoot = path.resolve(module.id, "../../../../test/__project");
        q = Q;
    } else {
        q = require('q');
        path = require('path');
        $ = require('jquery');
        targetRoot = path.resolve(__dirname, "../../../test/__project");
    }
    ...
    var getModId = function (curModule, subModule) {
        if (typeof window !== 'undefined') {
            return path.resolve(path.dirname(curModule), subModule).replace(targetRoot + '/', '').replace(/\\/g, '/');
        } else {
            return path.relative(targetRoot, path.resolve(path.dirname(curModule), subModule)).replace(/\\/g, '/');
        }
    };

    var getAMods = function () {
        if (typeof window !== 'undefined') {
            return require('../../../test/__project/amods');
        } else {
            return require(path.resolve(__dirname, "../../../test/__project/amods"));
        }
    };
    /**
     * todo:render函数数据来源问题
    ...
    var parseSync = function (htmlCode, subModules, defer, curModule) {
        ...
        syncSubs.forEach(function (sub) {
            var data = sub.selector;//todo:使用选择器选择的数据
            syncRender.push(
                getAMods()[getModId(curModule, sub.module)]
                    .render(data)
    ...
    var resolveAsync = function (asyncSubs, curModule) {
        ...
        asyncReplaceBegin.promise.then(function (errhandler) {
            ...
            asyncSubs.forEach(function (asub) { //使用jquery替换每一个异步返回的内容，注意此时可能代码还没有放到页面上，所以需要异步promise形式
                var subModule = getAMods()[getModId(curModule, asub.module)];
                subModule.render({})
                    .then(function (asubModule) { //runtime, add to dom
                        if ($('#' + asub.id).size() > 0) {
                            $('#' + asub.id).replaceWith(asubModule.html);
