...
define(function (require, exports, module) {
    ...
    var fillSyncSub = function (htmlCode, subModules, defer, curModule) {
        ...
        return q.all(syncRender).then(function () {
     ...
     * @param defer promise的defer对象
     * @param curModule 当前render模块的路径
     * @returns {Promise<U>} promise，直接返回htmlcode，然后逐步解析子模块，todo:使用jquer选择器替换到dom中的相应位置
     */
    var fillAsyncSub = function (htmlCode, subModules, defer, curModule) {
        ...
        asyncSubs.forEach(function (sub) {
            console.log('deal with async modules, use jquery replace the puzzle with render()=>htmlcode as dom elements');
            console.log(sub);
    ...
    /**
     * 解析此模块依赖的子模块
     * @param htmlCodeFn jstm产生的纯净的js模板方法
     * @param curModule 当前模块路径，此参数是为了加载模块依赖的子模块（子模块可能会使用相对路径）
     * @returns {Function}
     */
    exports.parseSubs = function (htmlCodeFn, curModule) {
