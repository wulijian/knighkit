...
define(function (require, exports, module) {
    ...
    var getDataFrom = function (propertyStr) {
        ...
        propertyStr.match(dataReg).forEach(function (dataAttr) {
            dataAttr = dataAttr.replace(/^data-/, '');
            var dataKey = dataAttr.match(/^(.*)=/);
            var dataValue = dataAttr.match(/=(.*)$/);
            if (dataKey !== null && dataValue !== null) {
                datas[dataKey[1]] = dataValue[1].match(/^(['"]?)(.*)\1$/)[2]; //string 去掉外面的 ' 和 ”
            }
        });
        //转化成json对象的形式
        return datas;
    ...
    /**
     * 获取是 layout 的模块，标志，含有name号（data-含有name号）属性
     * @param htmlCode
     * @returns {Array} 返回含有 data-name 的子模块，定义此类子模块为 layout，可在extend的时候被替换
     */
